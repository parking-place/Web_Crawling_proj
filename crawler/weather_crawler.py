...
def get_weather_data(browser, s_date, e_date):
    ...
    # 날짜 셀렉터에 시작 날짜 입력하기
    s_year_select.send_keys(f'{s_date[0]}')
    s_month_select.send_keys(f'{s_date[1]:02d}')
    s_day_select.send_keys(f'{s_date[2]:02d}')
    # 날짜 셀렉터에 종료 날짜 입력하기
    e_year_select.send_keys(f'{e_date[0]}')
    e_month_select.send_keys(f'{e_date[1]:02d}')
    e_day_select.send_keys(f'{e_date[2]:02d}')
    ...
    for start in starts:
        ...
        for i in range(2, 32):
            # 행 가져오기
            row = tbody_bs.select_one(row_css.format(i=i))
            # 행이 없으면 다음 월로 넘어가기
            if row is None:
                break
